...
const findHappiness = (arr) => {
  let reply = [];
  arr.map(find_smile => {
    if (find_smile.includes(':)')) reply.push(find_smile); 
    })
  return reply;
};
...
const standardizePhoneNumbers = (arr) => {
  let reply = [];
  for (var i = 0; i < arr.length; i++) {
    let section1 = arr[i].substring(1,4);
    let section2 = arr[i].substring(6,9);
    let section3 = arr[i].substring(10);
    reply.push(section1 + section2 + section3);
    console.log("1." + section1 + ".");
    console.log("1." + section2 + ".");
    console.log("1." + section3 + ".");
  }
return reply;
};
...
const onlyOddChars = (str) => {
  ...
  for (var i = 1; i < str.length; i = i + 2) {
      reply = reply + str.substring(i,i + 1);
      console.log("i: " + i);
