...
const removeOne = (num, arr) => {
  let remove_num = num;
  if (remove_num % 3 === 2) { arr.pop(remove_num);
  }
};
...
const removeWithForEach = (arr, helper) => {
  const step_thru = (num, idx, arr) => {
  helper(num, arr);
  };
  arr.forEach(step_thru);
  return arr;
};
...
const createList = (available_items) => {
  let store_inventory = []
    available_items.forEach(function(item){
    if(item.available == true) store_inventory.push(item.name)
  })
